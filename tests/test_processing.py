...
def test_process_design():
    ...
    # values = con.run(f'SELECT {', '.join(TABLE_LIST['design'])} FROM design LIMIT 2')
    # print(values)
...
def test_process_dates():
    dummy_data = {'sales_order': pd.DataFrame(data=[[2, datetime.datetime(2022, 11, 3, 14, 20, 52, 186000), datetime.datetime(2022, 11, 3, 14, 20, 52, 186000), 3, 19, 8, 42972, Decimal('3.94'), 2, '2022-11-07', '2022-11-08', 8]], columns=TABLE_LIST['sales_order'])}
...
def test_process_currency_data():
    ...
    assert list(df_currency.values[0]) == [1, 'GBP', 'United Kingdom Pound']
    assert list(df_currency.values[1]) == [2, 'USD', 'United States Dollar']
    assert list(df_currency.values[2]) == [3, 'EUR', "Euro Member Countries"]
...
def test_process_counterparty_data():
    ...
    # con = pg8000.native.Connection(config['USER'], host=config['HOST'], database=config['DATABASE'], port=config['PORT'], password=config['PASSWORD'])
    # values = con.run(f'SELECT {', '.join(TABLE_LIST['address'])} FROM address WHERE address_id=15')
    # print(values)
    # assert False
...
def test_process_staff_data():
    ...
    # con = pg8000.native.Connection(config['USER'], host=config['HOST'], database=config['DATABASE'], port=config['PORT'], password=config['PASSWORD'])
    # values = con.run(f'SELECT {', '.join(TABLE_LIST['department'])} FROM department where department_id=2')
    # print(values)
    # assert False
